...
-(void)viewDidLoad 
{
    ...
    // Range
    plotSpace.xRange = [CPPlotRange plotRangeWithLocation:CPDecimalFromFloat(1.0) length:CPDecimalFromFloat(5.0)];
    
    plotSpace.yRange = [CPPlotRange plotRangeWithLocation:CPDecimalFromFloat(1.0) length:CPDecimalFromFloat(500)];

    // Axes
	CPXYAxisSet *axisSet = (CPXYAxisSet *)graph.axisSet;
    CPXYAxis *x = axisSet.xAxis;
    x.majorIntervalLength = CPDecimalFromString(@"0.5");
    x.orthogonalCoordinateDecimal = CPDecimalFromString(@"40");
  ...
  */
   ...
   // Put an area gradient under the plot above
    ...
    for ( i = 0; i < 60; i++ ) {
        [contentArray addObject:[NSMutableDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithFloat:i], @"x",[NSNumber numberWithFloat:(300/i)^2], @"y",  nil]];
    }
